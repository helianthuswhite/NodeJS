...
function syntax_parse() {
	...
	while (stack_top >= 0) {
		if (token_curse >= TOKEN_SEQUENCE.length) {
			SYNTAX_RESULT.push('error:程序结构不完整，编译失败.');
			break;
		}
		...
		if (TERMINAL.indexOf(stack[stack_top]) > -1) {
			if (stack[stack_top] == TOKEN_SEQUENCE[token_curse])
				SYNTAX_RESULT.push('leaf:[' + TOKEN_SEQUENCE[token_curse] + ']');
			else SYNTAX_RESULT.push('error:不可接受的终结符：[' + TOKEN_SEQUENCE[token_curse] + ']');
			stack_top--;
			token_curse++;
		}
		else {
			if (PARSING_TABLE[stack[stack_top]][TOKEN_SEQUENCE[token_curse]] < 0) {
				...
				if (isTrue) {
					var tmp_str = 'success: [' + stack[stack_top] + ']\t->\t[Lambda]';
					SYNTAX_RESULT.push(tmp_str);
					stack_top--;
				}
				else {
					if (PARSING_TABLE[stack[stack_top]][TOKEN_SEQUENCE[token_curse]] == -1) {
						SYNTAX_RESULT.push('error: [' + TOKEN_SEQUENCE[token_curse] + 
				...
				var tmp_str = 'success: [' + stack[stack_top] + ']\t->\t';
				stack_top--;
